...
feature  -- Add tests
	...
	make
		...
		do
			...
			add_boolean_case (agent test_imps)
...
feature -- Tests
	test_imps: BOOLEAN
		...
		local
			d2: DICTIONARY[STRING, INTEGER]
		do
			create d2.make
			check
				attached {ARRAY[INTEGER]} d2.keys
			end
			check
				attached {LINKED_LIST[STRING]} d2.values
			end
			Result := True
		end
	...
	test_add: BOOLEAN
		...
		do
			...
			m1 := d.model
			entries := <<[7, "F"], [8, "G"]>>
			d.add_entries (create {SET[TUPLE[INTEGER, STRING]]}.make_from_array (entries))
			m2 := d.model
